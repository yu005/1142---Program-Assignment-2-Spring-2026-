// =====================================================
// 學生宿舍最佳配對系統（精簡版）
// 學生選偏好房型，系統自動計算偏好分數並配對
// 演算法：分支定界法 / 貪婪法 / 動態規劃
// =====================================================

let rooms = [];       // [{id, number, type(2/3/4)}]
let students = [];    // [{id, name, prefType}]
let bedSlots = [];    // [{slotIdx, roomIdx, roomNumber, type}]
let scoreMatrix = [];
...
// ========== Data Generation ==========
function generate() {
    const nd = Math.min(parseInt($('numDouble').value) || 0, 3);
    const nt = Math.min(parseInt($('numTriple').value) || 0, 2);
    const nq = Math.min(parseInt($('numQuad').value) || 0, 2);
    rooms = [];
    ...
    if (totalBeds === 0) { alert('請至少設定一間房間！'); return; }
    if (totalBeds > 10) { alert('總床位數建議不超過 10 個，以避免計算卡住。'); }

    // Generate students = total beds
    students = [];
...
function loadExample1() {
    ...
    expandBeds(); buildScoreMatrix(); renderAll();
...
function calcScore(student, slot) {
    // Base: random-seeded component (20-50) for variety
    const seed = (student.id * 31 + slot.slotIdx * 17 + 7) % 30 + 20;
    // Type match bonus (0-50)
    let typeBonus = 0;
...
function solveBnB() {
    ...
    function ub(person, used) {
        ...
        for (let p = person; p < sz; p++) {
            let mx = 0;
            for (let j = 0; j < m; j++) if (!used.has(j)) mx = Math.max(mx, scoreMatrix[p][j]);
            u += mx;
        }
        return u;
    }
    function search(p, sc, asgn, used) {
        nodesExplored++;
        if (p === sz) { if (sc > bestScore) { bestScore = sc; bestAssign = [...asgn]; } return; }
...
function solveGreedy() {
    ...
    let total = 0, steps = 0;
    const t0 = performance.now();
    for (const p of pairs) { steps++; if (usedI.has(p.i)||usedJ.has(p.j)) continue; asgn[p.i]=p.j; usedI.add(p.i); usedJ.add(p.j); total+=p.score; if(usedI.size===sz) break; }
    return { algo: '貪婪演算法', score: total, assignment: asgn, nodesExplored: steps, pruneCount: 0, time: performance.now() - t0 };
}
...
function solveDP() {
    ...
    const full = (1 << m) - 1;
    const dp = new Array(full + 1).fill(-1), par = new Array(full + 1).fill(-1);
    dp[0] = 0; let ne = 0;
    const t0 = performance.now();
    for (let mask = 0; mask <= full; mask++) {
        if (dp[mask] < 0) continue;
        const person = popcount(mask); if (person >= sz) continue;
        for (let j = 0; j < m; j++) {
            if (mask & (1 << j)) continue; ne++;
...
function runMatch() {
    ...
    let r; if (a==='bnb') r=solveBnB(); else if (a==='greedy') r=solveGreedy(); else r=solveDP();
...
function clearResults() {
    $('resultSection').style.display='none'; $('compareSection').style.display='none'; $('algoInfo').style.display='none';
    currentResult=null; document.querySelectorAll('.highlight-cell').forEach(c=>c.classList.remove('highlight-cell')); drawBipartite();
}

function displayResult(r) {
    $('resAlgo').textContent=r.algo; $('resTotalScore').textContent=r.score+' 分';
    $('resNodes').textContent=r.nodesExplored; $('resPrune').textContent=r.pruneCount;
    $('resTime').textContent=r.time.toFixed(2)+' ms';
    const det=$('matchDetails'); det.innerHTML='';
    // Group by room
    const rm = {};
    r.assignment.forEach((j,i)=>{ if(j<0) return; const s=bedSlots[j]; if(!rm[s.roomNumber]) rm[s.roomNumber]={type:s.type,members:[]}; rm[s.roomNumber].members.push({name:students[i].name,score:scoreMatrix[i][j]}); });
    ...
    const bp=findBlockingPairs(r.assignment); const sEl=$('resStability'), bpEl=$('blockingPairs'); bpEl.innerHTML='';
    if(!bp.length){sEl.textContent='✅ 穩定（無 blocking pair）';sEl.style.color='#059669';}
    else{sEl.textContent=`⚠️ ${bp.length} 個 blocking pair`;sEl.style.color='#dc2626'; bp.slice(0,4).forEach(b=>{const d=document.createElement('div');d.className='blocking-item';d.textContent=b.reason;bpEl.appendChild(d);});}
    ...
    [`節點：${r.nodesExplored}`,`修剪：${r.pruneCount}`,`最優：${r.score}`,`時間：${r.time.toFixed(2)}ms`].forEach((t,i)=>{const d=document.createElement('div');d.className='step-item'+(i===2?' highlight':'');d.textContent=t;ic.appendChild(d);});
...
function displayCompare(r1,r2,r3) {
    ...
    body.innerHTML='';
    [['總偏好分數',r1.score,r2.score,r3.score],['搜尋節點',r1.nodesExplored,r2.nodesExplored,r3.nodesExplored],
     ['修剪次數',r1.pruneCount,r2.pruneCount,r3.pruneCount],['時間(ms)',r1.time.toFixed(2),r2.time.toFixed(2),r3.time.toFixed(2)],
     ['最優?',r1.score===best?'✅':'❌',r2.score===best?'✅':'❌',r3.score===best?'✅':'❌']].forEach(row=>{
        const tr=document.createElement('tr'); if(row[0]==='總偏好分數') tr.className='best-row';
    ...
    const bpr=document.createElement('tr'); ['Blocking pairs',b1,b2,b3].forEach(c=>{const td=document.createElement('td');td.textContent=c;bpr.appendChild(td);}); body.appendChild(bpr);
...
function drawBipartite(assignment) {
    ...
    if(assignment){
        assignment.forEach((slotJ,i)=>{
            ...
            const ri=bedSlots[slotJ].roomIdx; if(ri>=rPos.length) return;
...
function drawGrid(){ctx.strokeStyle='#f1f5f9';ctx.lineWidth=1;for(let x=0;x<canvas.width;x+=50){ctx.beginPath();ctx.moveTo(x,0);ctx.lineTo(x,canvas.height);ctx.stroke();}for(let y=0;y<canvas.height;y+=50){ctx.beginPath();ctx.moveTo(0,y);ctx.lineTo(canvas.width,y);ctx.stroke();}}
