...
def create_report():
    ...
    # ==================== 摘要 ====================
    h1(doc, '摘要')
    p(doc, '本報告設計並實作一套學生宿舍最佳配對資訊系統。系統支援多種房間類型（雙人房、三人房、四人房），每位學生可選擇偏好的房間類型，系統自動計算偏好分數矩陣，並透過三種演算法——分支定界法（Branch-and-Bound）、貪婪演算法（Greedy Algorithm）與動態規劃法（Dynamic Programming）——計算最佳配對方案，使全體學生的總偏好分數最大化。')
    p(doc, '系統的核心策略是將多人房展開為個別床位槽（Bed Slot），將多對一的宿舍分配問題轉換為標準的一對一指派問題，再套用課堂所學的演算法求解。系統提供直覺的圖形化介面，包含可編輯的偏好分數矩陣、二部圖配對視覺化、三種演算法一鍵比較功能，以及穩定性檢測（Blocking Pair 偵測）。')
    doc.add_page_break()
    ...
    p(doc, '• 學生：共有 n 位學生申請住宿（n 等於總床位數），每位學生設定偏好的房間類型（雙人房 / 三人房 / 四人房 / 不限）。')
    p(doc, '• 偏好分數：系統根據學生偏好與房間類型的匹配度自動計算偏好分數（0-100），使用者也可手動微調。')
    p(doc, '• 配對目標：產生一個最佳的一對一分配方案（學生 ↔ 床位），使所有配對的總偏好分數最大化。')

    h2(doc, '1-2 範例資料')
    p(doc, '範例 1：2 間雙人房（房號 101、102），共 4 個床位，4 位學生。')
    add_table(doc,
        ['學生', '偏好房型', '101-1', '101-2', '102-1', '102-2'],
        [['A', '雙人房', '77', '77', '72', '72'],
         ['B', '雙人房', '68', '68', '73', '73'],
         ['C', '不限', '59', '59', '54', '54'],
         ['D', '雙人房', '90', '90', '55', '55']], '表 1  範例 1 偏好分數矩陣（4×4）')
    p(doc, '註：同一房間內的不同床位（如 101-1 和 101-2）對同一學生的偏好分數相同，因為兩個床位共享相同的房間屬性。')

    p(doc, '範例 2：1 間雙人房（201）+ 1 間三人房（202），共 5 個床位，5 位學生。')
    add_table(doc,
        ['學生', '偏好房型', '201-1', '201-2', '202-1', '202-2', '202-3'],
        [['A', '雙人房', '77', '77', '37', '37', '37'],
         ['B', '三人房', '38', '38', '78', '78', '78'],
         ['C', '雙人房', '89', '89', '39', '39', '39'],
         ['D', '不限', '50', '50', '60', '60', '60'],
         ['E', '三人房', '41', '41', '71', '71', '71']], '表 2  範例 2 偏好分數矩陣（5×5）')
    ...
    p(doc, '(1) 基礎分數（20-50 分）：根據學生與床位的編號組合產生的固定差異值，模擬不同學生對不同房間的個人偏好差異（例如地理位置、室友關係等無法量化的因素）。')
    p(doc, '(2) 房型匹配加分（0-50 分）：')
    add_table(doc,
        ['匹配情況', '加分'],
        [['完全符合（偏好房型 = 房間類型）', '+50'],
         ['偏好「不限」', '+30'],
         ['差一級（例如偏好雙人、實際三人）', '+20'],
         ['差兩級（例如偏好雙人、實際四人）', '+5']], '表 3  房型匹配加分規則')
    p(doc, '例如：學生 A 偏好「雙人房」，床位 101-1 屬於雙人房 → 基礎分 27 + 房型匹配加分 50 = 77 分。')
    ...
    add_table(doc,
        ['步驟', '使用者操作', '系統回應'],
        [['1', '設定雙人房/三人房/四人房數量', '—'],
         ['2', '點擊「生成」或載入範例', '建立房間、生成學生（含隨機偏好）、自動計算分數矩陣'],
         ['3', '（可選）調整每位學生的偏好房型', '即時重新計算偏好分數矩陣'],
    ...
    add_table(doc,
        ...
        [['FR-1.1', '多房型設定', '支援雙人房/三人房/四人房的混合配置，上限各 3/2/2 間'],
         ['FR-1.2', '學生偏好設定', '每位學生選擇偏好房型（雙人/三人/四人/不限）'],
         ['FR-1.3', '自動偏好分數計算', '根據房型匹配度自動計算 0-100 分'],
